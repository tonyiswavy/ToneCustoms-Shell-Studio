...
public sealed class NativeGtaExportService
{
 public async Task<NativeGtaExportResult> ExportAsync(ShellProject project,string output,string blenderExe,string bridgeScript,CancellationToken cancellationToken=default)
 {
  ...
  using var p=Process.Start(psi);if(p==null)return new(false,null,[],["Blender could not be started."]);var stdout=p.StandardOutput.ReadToEndAsync(cancellationToken);var stderr=p.StandardError.ReadToEndAsync(cancellationToken);await p.WaitForExitAsync(cancellationToken);await stdout;var err=await stderr;var status=Path.Combine(gtaOut,"bridge-status.json");if(!File.Exists(status))return new(false,null,[],["Blender did not produce bridge-status.json.",err]);
  using var doc=JsonDocument.Parse(await File.ReadAllTextAsync(status,cancellationToken));var root=doc.RootElement;var completed=root.TryGetProperty("gta_export_completed",out var done)&&done.GetBoolean();var files=root.TryGetProperty("gta_files",out var arr)&&arr.ValueKind==JsonValueKind.Array?arr.EnumerateArray().Select(x=>x.GetString()).Where(x=>!string.IsNullOrWhiteSpace(x)).Select(x=>Path.IsPathRooted(x!)?x!:Path.Combine(gtaOut,x!)).Where(File.Exists).ToList():[];
  ...
  issues.AddRange(new GtaAssetValidator().Validate(files).Where(x=>!x.Valid).Select(x=>$"{Path.GetFileName(x.File)}: {x.Detail}"));if(issues.Count>0)return new(false,null,files,issues);var package=new FiveMResourcePackager().Build(project,output,files);issues.AddRange(package.Issues);return new(package.Success,package.ResourcePath,files,issues);
 }
}
