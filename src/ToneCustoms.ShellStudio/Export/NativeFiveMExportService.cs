...
public sealed class NativeFiveMExportService
{
 public async Task<NativeExportResult> ExportAsync(ShellProject project,string blenderExe,string bridgeScript,string destination,CancellationToken ct=default)
 {
  ...
  using var p=Process.Start(psi);if(p==null)return new(false,null,["Blender failed to start."],[]);var stdout=p.StandardOutput.ReadToEndAsync(ct);var stderr=p.StandardError.ReadToEndAsync(ct);await p.WaitForExitAsync(ct);var err=await stderr;var statusFile=Path.Combine(gtaOut,"bridge-status.json");if(!File.Exists(statusFile))return new(false,null,["Blender/Sollumz did not create bridge-status.json.",err],[]);
  ...
  var nativeIssues=new GtaAssetValidator().ValidateFiles(gta).ToList();issues.AddRange(nativeIssues);if(!completed||p.ExitCode!=0||gta.Count==0){if(!string.IsNullOrWhiteSpace(err))issues.Add(err.Trim());if(gta.Count==0)issues.Add("No native GTA files were produced by Sollumz.");return new(false,null,issues.Distinct().ToList(),gta);}
